          ...
          ? [
              {
                ...
                createdAt: new Date(), // array: do NOT use serverTimestamp
                createdBy: user.uid,
...
const agentStats = (() => {
  ...
  leads.forEach((lead) => {
    if (!lead.assignedAgentName && !lead.assignedAgentEmail) return;

    const key = lead.assignedAgentEmail || lead.assignedAgentName;
    if (!map[key]) {
      map[key] = {
        name: lead.assignedAgentName || lead.assignedAgentEmail || "Unknown",
    ...
    // "Hot" = higher relationship rankings (tweak as you like)
    if (
...
async function handleCsvFileChange(e) {
  ...
  try {
    ...
    // Try both comma and semicolon (for some exports)
    ...
    // Detect columns by name (more forgiving)
    ...
    for (let i = 1; i < rows.length; i++) {
      ...
      // 3) Phone/email by detected headers
      ...
      // 4) Fallback: if we still have nothing, assume simple column order:
      //    Col0 = full name, Col1 = phone, Col2 = email
      ...
      // 5) FINAL fallback: scan *all* columns for an email/phone pattern
      if (!email) {
      ...
      if (!phone) {
        const phoneCandidate = cols.find(
          (c) => /\d/.test(c) && c.length >= 7 // “looks like” a phone-ish string
        );
        if (phoneCandidate) phone = phoneCandidate;
      }

      // If we STILL have nothing, skip row
      if (!firstName && !lastName && !email && !phone) {
        console.log("[CSV Import] Skipping row (no usable contact):", cols);
        continue;
      }

      // Build combined contact
      const contactParts = [];
      ...
      const payload = {
        ...
        // still store individually
        phone,
        email,

        // combined field for UI
        contact, // e.g. "267-555-1234 • test@example.com"
    ...
    setImporting(false);
    // Let user pick same file again if needed
    e.target.value = "";
  }
}
...
{filteredLeads.map((lead) => (
  ...
  >
    {/* rest of your row stays the same */}
  </tr>
))}
              ...
              {leads.map((lead) => (
                ...
                >
                  ...
                  <td className="px-3 py-2 align-top text-[11px]">
                    {(() => {
                      const fromLastEntry = lead.journalLastEntry;
                      let fromArray = "";
                      if (Array.isArray(lead.journal) && lead.journal.length > 0) {
                        const last = lead.journal[lead.journal.length - 1];
                        fromArray = last?.text || "";
                      }
                      const latest = fromLastEntry || fromArray;
                      ...
                      return <div className="text-gray-800">{latest}</div>;
                    })()}
                  </td>
